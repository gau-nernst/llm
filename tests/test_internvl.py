...
def test_internvl():
    ...
    torch.testing.assert_close(out, out_ref, rtol=2e-4, atol=2e-4)
    ...
    input_ids = torch.cat(
        [
            torch.randint(0, max_token_id, size=(1, 16), device=device),
            torch.tensor([img_tokens], device=device),
            torch.randint(0, max_token_id, size=(1, 8), device=device),
        ],
        dim=1,
    )
    image_flags = torch.ones(1, num_img_tokens, device="cuda")
